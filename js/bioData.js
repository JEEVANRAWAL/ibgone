$(document).ready(function() {
    ...
    function fetchSingleUser(userId) {
        $.ajax({
            url: "http://localhost/php/ibg/costume-apis/api-fetch-biodata.php?id=" + userId,
            type: "GET",
            success: function(data) {
                ...
                $.each(data[0], function(key, value) {
                    if (key == 'img_url') {
                        console.log("the value of img_url : " + "../../" + value);
                        // $('#profile_pic').css("background-image", "url(../../" + value + ")");
                        $("#profile_pic").attr("src", "../../" + value);
                    }
                    $(`#${key}`).html(value);
                });
            }
